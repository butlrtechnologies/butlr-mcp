...
export const TrafficFlowArgsSchema = z
  .object({
    space_id_or_name: z.string().min(1, "space_id_or_name cannot be empty").max(200).trim(),

    time_window: z.enum(["20m", "1h", "today", "custom"]).default("today"),

    custom_start: z.string().pipe(timeStringSchema).optional(),

    custom_stop: z.string().pipe(timeStringSchema).optional(),
