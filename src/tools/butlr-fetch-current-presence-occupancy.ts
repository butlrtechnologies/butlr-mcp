...
/**
 * Tool definition for butlr_fetch_current_presence_occupancy
 */
export const currentPresenceOccupancyTool = {
  name: "butlr_fetch_current_presence_occupancy",
  description:
    "Get current presence-based occupancy (last 5 minutes median) for rooms, zones, or floors. " +
    "Supports zones (traffic doesn't). Uses 60s cache for performance.",
  inputSchema: {
    type: "object",
    properties: {
      asset_ids: {
...
/**
 * Input arguments for butlr_fetch_current_presence_occupancy
 */
export interface CurrentPresenceOccupancyArgs {
  asset_ids: string[];
}

/**
 * Execute butlr_fetch_current_presence_occupancy tool
 */
export async function executeCurrentPresenceOccupancy(args: CurrentPresenceOccupancyArgs) {
  // Validate inputs
  ...
  if (process.env.DEBUG) {
    console.error(
      `[butlr-fetch-current-presence-occupancy] Querying ${args.asset_ids.length} assets`
    );
  }
  ...
  const cachedData = Object.values(hits).map((entry) => ({
    ...
    measurement: `${entry.asset_type}_occupancy`,
  }));
  ...
  if (allOccupancy.length === 0) {
    response.note =
      "No current presence data. Assets may not have presence sensors or no recent activity (last 5 minutes). " +
      "For traffic-based measurements (entry/exit counts), try butlr_fetch_current_traffic_occupancy.";
  }
