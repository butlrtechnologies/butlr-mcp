...
- `isProductionSensor` no longer treats a blank or null `mac_address` as a test device. Test devices are identified by MAC prefix (`mi-rr-or`, `fa-ke`), so a missing MAC is unknown provenance, not evidence, and a real sensor with no MAC recorded was hard rejected by `butlr_traffic_flow` as a "test/mirror device". Sensors are addressed by `id` everywhere, so a blank MAC never made a row unqueryable. `isProductionHive` is unchanged: a hive's serial number IS its addressing key.
...
- `Sensor.mode` is now optional in the client types. The non-optional union made TypeScript narrow `mode !== "traffic"` to `"presence"` and marked the tools' missing-mode guards unreachable, so a null mode from the API would have rendered as "a null-mode sensor" with no live guard.
